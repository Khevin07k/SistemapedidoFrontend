...
async function funIngresar() {
	try {
		// Llamada explícita al método login del store
		const response = await authStore.login(user.value);
		// console.log(response);
		if (response) {
			router.push("/");
		} else {
			// Manejo de errores si la respuesta no es válida
		...
		errorStatus.value = true;
		errorMessage.value = "Ocurrió un error al intentar iniciar sesión. Por favor, intenta más tarde.";
		localStorage.clear(); // Limpiar localStorage en caso de error
	}
}
/*async function funIngresar() {
	try {
		const response = await authServices.login(user.value);
			...
			<div
				...
				<div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
					...
					<form @submit.prevent="funIngresar">
						<FloatLabel>
							<InputText id="email" type="text" class="w-full" v-model="user.email" />
							<label for="email">Email</label>
						</FloatLabel>
						<br>
						<FloatLabel>
							<Password id="password" v-model="user.password" class="w-full" fluid :feedback="false">
							</Password>
							<label for="password">Contraseña</label>
						</FloatLabel>
						<br>
						<input type="submit" value="Iniciar Sesion" class="w-full"></input>
					</form>

					<br>
					<div class="flex items-center justify-between mt-2 mb-8 gap-8">
						<span class="font-medium no-underline ml-2 text-right">¿No tienes cuenta?</span>
						<router-link to="register"
							class="font-medium no-underline ml-2 text-left cursor-pointer text-primary">
							Registrarse
						</router-link>
					</div>
					<!--					<br>-->
